...
!(function(a, b) {
  ...
  function d(d, e, f) {
    const g = d.children(e.headerTag);
    const h = d.children(e.bodyTag);
    g.length > h.length
      ? R(Z, 'contents')
      : g.length < h.length && R(Z, 'titles');
    let i = e.startIndex;
    if (((f.stepCount = g.length), e.saveState && a.cookie)) {
      const j = a.cookie(U + q(d));
    ...
    (f.currentIndex = i),
      g.each(function(e) {
        ...
        const k = j === $.html || g.data('url') === b ? '' : g.data('url');
        const l = j !== $.html && g.data('loaded') === '1';
        const m = a.extend({}, bb, {
          title: f.html(),
  ...
  function h(a, b) {
    const c = a.find('.steps li').eq(b.currentIndex);
  ...
  function i(a) {
    let b = a.data('eventNamespace');
    return b == null && ((b = `.${q(a)}`), a.data('eventNamespace', b)), b;
  }
  function j(a, b) {
    const c = q(a);
    return a.find(`#${c}${V}${b}`);
  ...
  function q(a) {
    ...
    return (
      b == null &&
        ((b = a._id()),
        b == null && ((b = 'steps-uid-'.concat(T)), a._id(b)),
        T++,
        a.data('uid', b)),
      b
    );
  ...
  function r(a, c) {
    if ((S('enumType', a), S('keyOrValue', c), typeof c === 'string')) {
      const d = a[c];
      return d === b && R("The enum key '{0}' does not exist.", c), d;
    }
    if (typeof c === 'number') {
      for (const e in a) if (a[e] === c) return c;
  ...
  function w(b) {
    ...
    return this.each(function() {
      ...
      b.data('options', c),
        ...
        J(b, c, e),
        G(b, c),
        c.autoFocus && T === 0 && j(b, c.startIndex).focus(),
        b.triggerHandler('init', [c.startIndex]);
    });
  }
  function x(b, c, d, e, f) {
    (e < 0 || e > d.stepCount) && R(Y),
      (f = a.extend({}, bb, f)),
      y(b, e, f),
      ...
      d.stepCount++;
    const g = b.find('.content');
    const h = a('<{0}>{1}</{0}>'.format(c.headerTag, f.title));
    const i = a('<{0}></{0}>'.format(c.bodyTag));
    return (
      (f.contentMode == null || f.contentMode === $.html) && i.html(f.content),
      e === 0
        ...
        switch (r($, f.contentMode)) {
          case $.iframe:
            b.find('.content > .body')
              ...
              .empty()
              .html(
                `<iframe src="${f.contentUrl}" frameborder="0" scrolling="no" />`
              )
              .data('loaded', '1');
            break;
          case $.async:
  ...
  function D(a, b, c) {
    if (b.enablePagination) {
      const d = a.find(".actions a[href$='#finish']").parent();
      const e = a.find(".actions a[href$='#next']").parent();
      if (!b.forceMoveForward) {
        const f = a.find(".actions a[href$='#previous']").parent();
      ...
      b.enableFinishButton && b.showFinishButtonAlways
        ...
        : (d._showAria(
          ...
          e
            ._showAria(c.stepCount === 0 || c.stepCount > c.currentIndex + 1)
            ._enableAria(
  ...
  function E(b, c, d, e) {
    const f = j(b, d.currentIndex);
    let g = a(`<span class="current-info audible">${c.labels.current} </span>`);
    const h = b.find('.content > .title');
    if (e != null) {
      const i = j(b, e);
      i
        .parent()
  ...
  function F(a, b, c, d) {
    for (let e = q(a), f = d; f < c.stepCount; f++) {
      const g = e + V + f;
      const h = e + W + f;
      const i = e + X + f;
      const j = a
  ...
  function H(a, b, c, d) {
    return d < 0 || d >= c.stepCount || c.currentIndex === d
      ? !1
      : (I(a, d),
  ...
  function J(b, c, d) {
    const e = '<{0} class="{1}">{2}</{0}>';
    const f = r(_, c.stepsOrientation);
    const g = f === _.vertical ? ' vertical' : '';
    const h = a(
      e.format(c.contentContainerTag, `content ${c.clearFixCssClass}`, b.html())
    );
    const i = a(
      e.format(
        c.stepsContainerTag,
    ...
    const j = h.children(c.headerTag);
    const k = h.children(c.bodyTag);
    b
  ...
  function L(a, b, c) {
    if (b.enablePagination) {
      ...
      const e = '<li><a href="#{0}" role="menuitem">{1}</a></li>';
      let f = '';
  ...
  function M(a, c) {
    for (let d = a.match(/#([a-z]*)#/gi), e = 0; e < d.length; e++) {
      const f = d[e];
      const g = f.substring(1, f.length - 1);
      c[g] === b &&
  ...
  function N(b, c, d, e, f) {
    ...
    const j = g + W + f;
    const k = g + X + f;
    const l = b.find('.steps > ul');
    const m = M(c.titleTemplate, { index: f + 1, title: e.html() });
    const n = a(
      `<li role="tab"><a id="${h}" href="#${k}" aria-controls="${j}">${m}</a></li>`
    );
  ...
  function P(b, c, d, e, f, g) {
    const h = b.find('.content > .body');
    const i = r(ab, c.transitionEffect);
    const j = c.transitionEffectSpeed;
    const k = h.eq(e);
    const l = h.eq(f);
    switch (i) {
      case ab.fade:
      case ab.slide:
        ...
        var o = i === ab.fade ? 'fadeIn' : 'slideDown';
  ...
  function Q(b) {
    ...
    const e = m(d);
    const f = n(d);
    const g = f.currentIndex;
    if (c.parent().is(':not(.disabled):not(.current)')) {
      const h = c.attr('href');
      const i = parseInt(h.substring(h.lastIndexOf('-') + 1), 0);
      u(d, e, f, i);
    }
  ...
  function S(a, b) {
    b == null && R("The argument '{0}' is null or undefined.", a);
  }
  a.fn.extend({
    _aria: function(a, b) {
    ...
    String.prototype.format ||
      (String.prototype.format = function() {
        for (
          var b =
              arguments.length === 1 && a.isArray(arguments[0])
                ? arguments[0]
                : arguments,
            c = this,
            d = 0;
          ...
          const e = new RegExp(`\\{${d}\\}`, 'gm');
          c = c.replace(e, b[d]);
        }
        return c;
      });
  var T = 0;
  var U = 'jQu3ry_5teps_St@te_';
  var V = '-t-';
  var W = '-p-';
  var X = '-h-';
  var Y = 'Index out of range.';
  var Z = 'One or more corresponding step {0} are missing.';
  ...
  var _ = (a.fn.steps.stepsOrientation = { horizontal: 0, vertical: 1 });
  var ab = (a.fn.steps.transitionEffect = {
    none: 0,
    fade: 1,
    slide: 2,
    slideLeft: 3
  });
  var bb = (a.fn.steps.stepModel = {
    title: '',
    content: '',
    contentUrl: '',
    contentMode: $.html,
