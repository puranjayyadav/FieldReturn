...
$(function() {
  ...
  const toggle_sticky_header = function(sticky) {
    if (!sticky) {
  ...
  $('#mini_sidebar_setting').on('change', function() {
    const _val = $(this).is(':checked') ? 'checked' : 'unchecked';
    if (_val === 'checked') {
      toggle_sidebar_mini(true);
    } else {
      toggle_sidebar_mini(false);
  ...
  $(".select-sidebar[value|='2']").addClass('active');
